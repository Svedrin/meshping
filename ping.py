...
"""
    ...
    Cloned from Samuel Stauffer's GitHub repo:
    ...
    - moved gethostbyname call to verbose_ping (I want to use getaddrinfo
      instead)
...
def receive_one_ping(my_socket, timeout, sent_at={}):
    ...
    while True:
        ...
        if type == 8:
            ...
            packetID = None
            if type == 3:
                # we recv'ed an "unreachable" failure of some sort. the original
...
def do_one(dest_addr, target_id, seq, timeout):
    """
    ...
    my_ID = (os.getpid() + target_id) & 0xFFFF

    try:
        send_one_ping(my_socket, dest_addr, my_ID, seq)
        delay = None
        while True:
            response = receive_one_ping(my_socket, timeout)
            if response["timeout"]:
                return None
            if response["success"]:
                if response["id"] == my_ID and response["seq"] == seq:
                    return response["delay"]
            # looks like an error then. see if it concerns us
            if response.get("id", None) != my_ID or response.get("seq", None) != seq:
                continue
...
def verbose_ping(dest_addr, target_id, timeout = 2, count = 4):
    """
    ...
    for i in xrange(count):
        ...
        try:
            delay  =  do_one(socket.gethostbyname(dest_addr), target_id, i, timeout)
        except socket.gaierror, e:
            print "failed. (socket error: '%s')" % e[1]
            break
        except RuntimeError, e:
            print "failed. (runtime error: %s)" % e
            break
...
if __name__ == '__main__':
    verbose_ping("heise.de", 1)
    verbose_ping("google.com", 2)
    verbose_ping("a-test-url-taht-is-not-available.com", 3)
    verbose_ping("192.168.1.1", 4)
    verbose_ping("192.168.0.20", 5)
